...
namespace vFrame.Lockstep.Core.FarseerPhysics.Common
{
    ...
    public class Path
    {
        ...
        /// </summary>
        [XmlElement("ControlPoints")] public List<TSVector2> ControlPoints;
        ...
        public void Rotate(FixedPoint value) {
            ...
            for (int i = 0; i < ControlPoints.Count; i++)
                ControlPoints[i] = TSVector2.Transform(ControlPoints[i], rotationMatrix);
        }
        ...
        public Vertices GetVertices(int divisions) {
            ...
            FixedPoint timeStep = 1f / divisions;

            for (FixedPoint i = 0; i < 1f; i += timeStep) {
        ...
        public TSVector2 GetPositionNormal(FixedPoint time) {
            FixedPoint offsetTime = time + 0.0001f;
...
output = new TSVector2();
        ...
        public List<TSVector> SubdivideEvenly(int divisions) {
            ...
            FixedPoint deltaLength = length / divisions + 0.001f;
            FixedPoint t = 0.000f;
            ...
            for (int i = 1; i < divisions; i++) {
                ...
                FixedPoint angle = (FixedPoint) TSMath.Atan2(normal.y, normal.x);

                verts.Add(new TSVector(end, angle));
