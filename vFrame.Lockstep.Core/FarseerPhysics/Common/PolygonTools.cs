    ...
    public static class PolygonTools
    {
        ...
        public static Vertices CreateRoundedRectangle(FixedPoint width, FixedPoint height, FixedPoint xRadius, FixedPoint yRadius,
            ...
            if (segments == 0) {
                ...
                FixedPoint stepSize = MathHelper.TwoPi / (numberOfEdges - 4);
                ...
                for (int i = 1; i < numberOfEdges; i++) {
                    ...
                    vertices.Add(posOffset + new TSVector2(xRadius * (FixedPoint) TSMath.Cos(stepSize * -(i + phase)),
                                     -yRadius * (FixedPoint) TSMath.Sin(stepSize * -(i + phase))));
                }
            }
        ...
        public static Vertices CreateEllipse(FixedPoint xRadius, FixedPoint yRadius, int numberOfEdges) {
            ...
            FixedPoint stepSize = MathHelper.TwoPi / numberOfEdges;

            vertices.Add(new TSVector2(xRadius, 0));
            for (int i = numberOfEdges - 1; i > 0; --i)
                vertices.Add(new TSVector2(xRadius * (FixedPoint) TSMath.Cos(stepSize * i),
                    -yRadius * (FixedPoint) TSMath.Sin(stepSize * i)));
        ...
        public static Vertices CreateArc(FixedPoint radians, int sides, FixedPoint radius) {
            ...
            for (int i = sides - 1; i > 0; i--) {
                vertices.Add(new TSVector2(radius * (FixedPoint) TSMath.Cos(stepSize * i),
                    radius * (FixedPoint) TSMath.Sin(stepSize * i)));
            }
        ...
        public static Vertices CreateCapsule(FixedPoint height, FixedPoint topRadius, int topEdges, FixedPoint bottomRadius,
            ...
            FixedPoint newHeight = (height - topRadius - bottomRadius) * 0.5f;

            // top
            vertices.Add(new TSVector2(topRadius, newHeight));

            FixedPoint stepSize = MathHelper.Pi / topEdges;
            for (int i = 1; i < topEdges; i++) {
                vertices.Add(new TSVector2(topRadius * (FixedPoint) TSMath.Cos(stepSize * i),
                    topRadius * (FixedPoint) TSMath.Sin(stepSize * i) + newHeight));
            }
            ...
            for (int i = 1; i < bottomEdges; i++) {
                vertices.Add(new TSVector2(-bottomRadius * (FixedPoint) TSMath.Cos(stepSize * i),
                    -bottomRadius * (FixedPoint) TSMath.Sin(stepSize * i) - newHeight));
            }
        ...
        public static Vertices CreateGear(FixedPoint radius, int numberOfTeeth, FixedPoint tipPercentage, FixedPoint toothHeight) {
            ...
            FixedPoint stepSize = MathHelper.TwoPi / numberOfTeeth;
            tipPercentage /= 100f;
            MathHelper.Clamp(tipPercentage, 0f, 1f);
            FixedPoint toothTipStepSize = (stepSize / 2f) * tipPercentage;

            FixedPoint toothAngleStepSize = (stepSize - (toothTipStepSize * 2f)) / 2f;

            for (int i = numberOfTeeth - 1; i >= 0; --i) {
                if (toothTipStepSize > 0f) {
                    vertices.Add(
                        new TSVector2(radius *
                                     (FixedPoint) TSMath.Cos(stepSize * i + toothAngleStepSize * 2f + toothTipStepSize),
                            -radius *
                            (FixedPoint) TSMath.Sin(stepSize * i + toothAngleStepSize * 2f + toothTipStepSize)));

                    vertices.Add(
                        new TSVector2((radius + toothHeight) *
                                     (FixedPoint) TSMath.Cos(stepSize * i + toothAngleStepSize + toothTipStepSize),
                            -(radius + toothHeight) *
                            (FixedPoint) TSMath.Sin(stepSize * i + toothAngleStepSize + toothTipStepSize)));
                }

                vertices.Add(new TSVector2((radius + toothHeight) *
                                          (FixedPoint) TSMath.Cos(stepSize * i + toothAngleStepSize),
                    -(radius + toothHeight) *
                    (FixedPoint) TSMath.Sin(stepSize * i + toothAngleStepSize)));
        ...
        /// <param name="holeDetection">if set to <c>true</c> it will perform hole detection.</param>
        /// <returns></returns>
        public static List<Vertices> CreatePolygon(uint[] data, int width, FixedPoint hullTolerance,
