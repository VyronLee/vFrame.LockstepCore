...
namespace vFrame.Lockstep.Core.FarseerPhysics.Common.Decomposition
{
    ...
    {
        private static readonly FixedPoint PiSlop = 3.1f;
        ...
        private FixedPoint Angle(Point p) {
            Point a = (p.Next - p);
            Point b = (p.Prev - p);
            return (FixedPoint) TSMath.Atan2(a.Cross(b), a.Dot(b));
        }

        private bool AngleSign() {
            Point a = (_head.Next - _head);
            Point b = (_tail - _head);
            return TSMath.Atan2(a.Cross(b), a.Dot(b)) >= 0;
        }
    ...
    internal class Triangulator
    {
        ...
        private FixedPoint _sheer = 0.001f;
