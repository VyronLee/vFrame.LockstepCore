...
using vFrame.Lockstep.Core.FarseerPhysics.Common.PolygonManipulation;
...
namespace vFrame.Lockstep.Core.FarseerPhysics.Common.Decomposition
{
    ...
    public static class EarclipDecomposer
    {
        ...
        private static readonly FixedPoint Tol = .001f;
        ...
        /// <returns></returns>
        public static List<Vertices> ConvexPartition(Vertices vertices, int maxPolys, FixedPoint tolerance) {
            if (vertices.Count < 3)
        ...
        ///<param name="tolerance">The tolerance</param>
        ///<returns></returns>
        public static List<Vertices> PolygonizeTriangles(List<Triangle> triangulated, int maxPolys, FixedPoint tolerance) {
            ...
            for (int i = 0; i < triangulated.Count; ++i) {
                ...
                if (((triangulated[i].X[0] == triangulated[i].X[1]) && (triangulated[i].Y[0] == triangulated[i].Y[1]))
                    ...
                    ||
                    ((triangulated[i].X[0] == triangulated[i].X[2]) &&
                     (triangulated[i].Y[0] == triangulated[i].Y[2]))) {
                    covered[i] = true;
                }
            }
            ...
            while (notDone) {
                ...
                if (currTri == -1) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        poly.Add(new TSVector2(triangulated[currTri].X[i], triangulated[currTri].Y[i]));
                    }
        ...
        public static List<Triangle> TriangulatePolygon(Vertices vertices) {
            ...
            while (vNum > 3) {
                // Find an ear
                int earIndex = -1;
                FixedPoint earMaxMinCross = -10.0f;
        ...
        private static bool ResolvePinchPoint(Vertices pin, out Vertices poutA, out Vertices poutB) {
            ...
            for (int i = 0; i < pin.Count; ++i) {
                for (int j = i + 1; j < pin.Count; ++j) {
                    ...
                    if (TSMath.Abs(pin[i].x - pin[j].x) < Tol && TSMath.Abs(pin[i].y - pin[j].y) < Tol && j != i + 1) {
                        pinchIndexA = i;
                        pinchIndexB = j;
                        hasPinchPoint = true;
                        break;
        ...
        private static Vertices AddTriangle(Triangle t, Vertices vertices) {
            ...
            for (int i = 0; i < vertices.Count; i++) {
                ...
                if (i == firstP)
                    result.Add(new TSVector2(t.X[tipT], t.Y[tipT]));
            }
    ...
    public class Triangle
    {
        ...
        public Triangle(FixedPoint x1, FixedPoint y1, FixedPoint x2, FixedPoint y2, FixedPoint x3, FixedPoint y3) {
            X = new FixedPoint[3];
            Y = new FixedPoint[3];
            FixedPoint dx1 = x2 - x1;
            FixedPoint dx2 = x3 - x1;
            FixedPoint dy1 = y2 - y1;
            FixedPoint dy2 = y3 - y1;
            FixedPoint cross = dx1 * dy2 - dx2 * dy1;
            bool ccw = (cross > 0);
            if (ccw) {
                X[0] = x1;
                X[1] = x2;
        ...
        public bool IsInside(FixedPoint x, FixedPoint y) {
            ...
            FixedPoint vx2 = x - X[0];
            FixedPoint vy2 = y - Y[0];
            FixedPoint vx1 = X[1] - X[0];
            FixedPoint vy1 = Y[1] - Y[0];
            FixedPoint vx0 = X[2] - X[0];
            FixedPoint vy0 = Y[2] - Y[0];
            ...
            FixedPoint invDenom = 1.0f / (dot00 * dot11 - dot01 * dot01);
