...
namespace vFrame.Lockstep.Core.FarseerPhysics.Collision
{
    ...
    public static class TimeOfImpact
    {
        ...
        public static void CalculateTimeOfImpact(out TOIOutput output, TOIInput input) {
            ...
            FixedPoint target = TSMath.Max(Settings.LinearSlop, totalRadius - 3.0f * Settings.LinearSlop);
            FixedPoint tolerance = 0.25f * Settings.LinearSlop;
            ...
            for (;;) {
                ...
                for (;;) {
                    ...
                    for (;;) {
                        ...
                        Debug.Assert(!FixedPoint.IsNaN(t));

                        FixedPoint s = SeparationFunction.Evaluate(indexA, indexB, t);

                        Debug.Assert(!FixedPoint.IsNaN(s));

                        if (TSMath.Abs(s - target) < tolerance) {
