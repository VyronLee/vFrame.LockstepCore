...
namespace vFrame.Lockstep.Core.FarseerPhysics.Collision
{
    ...
    public class QuadTree<T>
    {
        ...
        /// <returns></returns>
        public static bool RayCastAABB(AABB aabb, TSVector2 p1, TSVector2 p2) {
            AABB segmentAABB = new AABB();
            {
                TSVector2.Min(ref p1, ref p2, out segmentAABB.LowerBound);
                TSVector2.Max(ref p1, ref p2, out segmentAABB.UpperBound);
            }
            if (!AABB.TestOverlap(aabb, segmentAABB)) return false;

            TSVector2 rayDir = p2 - p1;
            TSVector2 rayPos = p1;

            TSVector2 norm = new TSVector2(-rayDir.y, rayDir.x); //normal to ray
            if (norm.magnitude == 0.0)
                return true; //if ray is just a point, return true (iff point is within aabb, as tested earlier)
            norm.Normalize();

            FixedPoint dPos = TSVector2.Dot(rayPos, norm);

            TSVector2[] verts = aabb.GetVertices();
            FixedPoint d0 = TSVector2.Dot(verts[0], norm) - dPos;
            for (int i = 1; i < 4; i++) {
                FixedPoint d = TSVector2.Dot(verts[i], norm) - dPos;
                if (TSMath.Sign(d) != TSMath.Sign(d0))
        ...
        public void RayCast(Func<RayCastInput, Element<T>, FixedPoint> callback, ref RayCastInput input) {
            Stack<QuadTree<T>> stack = new Stack<QuadTree<T>>();
            stack.Push(this);

            FixedPoint maxFraction = input.MaxFraction;
            TSVector2 p1 = input.Point1;
            TSVector2 p2 = p1 + (input.Point2 - input.Point1) * maxFraction;
            ...
            while (stack.Count > 0) {
                ...
                foreach (Element<T> n in qt.Nodes) {
                    ...
                    FixedPoint value = callback(subInput, n);
                    if (value == 0.0f)
