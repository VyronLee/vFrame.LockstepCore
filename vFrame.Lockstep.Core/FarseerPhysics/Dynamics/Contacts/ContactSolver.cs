...
namespace vFrame.Lockstep.Core.FarseerPhysics.Dynamics.Contacts
{
    public sealed class ContactPositionConstraint
    {
        ...
        public int indexA;
        public int indexB;
        public FixedPoint invMassA, invMassB;
        public TSVector2 localCenterA, localCenterB;
        public FixedPoint invIA, invIB;
        public ManifoldType type;
        public FixedPoint radiusA, radiusB;
    ...
    public sealed class VelocityConstraintPoint
    {
        ...
        public FixedPoint normalMass;
        public FixedPoint tangentMass;
        public FixedPoint velocityBias;
    }
    ...
    public sealed class ContactVelocityConstraint
    {
        ...
        public FixedPoint tangentSpeed;
        public int pointCount;
    ...
    public class ContactSolver
    {
        ...
        public void Reset(TimeStep step, int count, Contact[] contacts, Position[] positions, Velocity[] velocities) {
            ...
            for (int i = 0; i < _count; ++i) {
                ...
                FixedPoint radiusA = shapeA.Radius;
                FixedPoint radiusB = shapeB.Radius;
        ...
        public void InitializeVelocityConstraints() {
            for (int i = 0; i < _count; ++i) {
                ...
                FixedPoint radiusA = pc.radiusA;
                FixedPoint radiusB = pc.radiusB;
                Manifold manifold = _contacts[vc.contactIndex].Manifold;

                int indexA = vc.indexA;
                int indexB = vc.indexB;

                FixedPoint mA = vc.invMassA;
                FixedPoint mB = vc.invMassB;
                FixedPoint iA = vc.invIA;
                FixedPoint iB = vc.invIB;
                TSVector2 localCenterA = pc.localCenterA;
                TSVector2 localCenterB = pc.localCenterB;

                TSVector2 cA = _positions[indexA].c;
                FixedPoint aA = _positions[indexA].a;
                TSVector2 vA = _velocities[indexA].v;
                FixedPoint wA = _velocities[indexA].w;

                TSVector2 cB = _positions[indexB].c;
                FixedPoint aB = _positions[indexB].a;
                TSVector2 vB = _velocities[indexB].v;
                FixedPoint wB = _velocities[indexB].w;
                ...
                for (int j = 0; j < pointCount; ++j) {
                    ...
                    FixedPoint rnA = MathUtils.Cross(vcp.rA, vc.normal);
                    FixedPoint rnB = MathUtils.Cross(vcp.rB, vc.normal);

                    FixedPoint kNormal = mA + mB + iA * rnA * rnA + iB * rnB * rnB;

                    vcp.normalMass = kNormal > 0.0f ? 1.0f / kNormal : 0.0f;

                    TSVector2 tangent = MathUtils.Cross(vc.normal, 1.0f);

                    FixedPoint rtA = MathUtils.Cross(vcp.rA, tangent);
                    FixedPoint rtB = MathUtils.Cross(vcp.rB, tangent);

                    FixedPoint kTangent = mA + mB + iA * rtA * rtA + iB * rtB * rtB;

                    vcp.tangentMass = kTangent > 0.0f ? 1.0f / kTangent : 0.0f;

                    // Setup a velocity bias for restitution.
                    vcp.velocityBias = 0.0f;
                    FixedPoint vRel = TSVector2.Dot(vc.normal,
                ...
                if (vc.pointCount == 2) {
                    ...
                    FixedPoint rn1A = MathUtils.Cross(vcp1.rA, vc.normal);
                    FixedPoint rn1B = MathUtils.Cross(vcp1.rB, vc.normal);
                    FixedPoint rn2A = MathUtils.Cross(vcp2.rA, vc.normal);
                    FixedPoint rn2B = MathUtils.Cross(vcp2.rB, vc.normal);

                    FixedPoint k11 = mA + mB + iA * rn1A * rn1A + iB * rn1B * rn1B;
                    FixedPoint k22 = mA + mB + iA * rn2A * rn2A + iB * rn2B * rn2B;
                    FixedPoint k12 = mA + mB + iA * rn1A * rn2A + iB * rn1B * rn2B;

                    // Ensure a reasonable condition number.
                    FixedPoint k_maxConditionNumber = 1000.0f;
        ...
        public void SolveVelocityConstraints() {
            for (int i = 0; i < _count; ++i) {
                ...
                TSVector2 normal = vc.normal;
                TSVector2 tangent = MathUtils.Cross(normal, 1.0f);
                FixedPoint friction = vc.friction;
                ...
                for (int j = 0; j < pointCount; ++j) {
                    ...
                    FixedPoint vt = TSVector2.Dot(dv, tangent) - vc.tangentSpeed;
                    FixedPoint lambda = vcp.tangentMass * (-vt);

                    // b2Clamp the accumulated force
                    FixedPoint maxFriction = friction * vcp.normalImpulse;
                    FixedPoint newImpulse = MathUtils.Clamp(vcp.tangentImpulse + lambda, -maxFriction, maxFriction);
                ...
                if (vc.pointCount == 1) {
                    ...
                    FixedPoint vn = TSVector2.Dot(dv, normal);
                    FixedPoint lambda = -vcp.normalMass * (vn - vcp.velocityBias);

                    // b2Clamp the accumulated impulse
                    FixedPoint newImpulse = TSMath.Max(vcp.normalImpulse + lambda, 0.0f);
                    ...
                    FixedPoint vn1 = TSVector2.Dot(dv1, normal);
                    FixedPoint vn2 = TSVector2.Dot(dv2, normal);
                    ...
                    //static readonly FixedPoint k_errorTol = 1e-3f;
        ...
        public bool SolvePositionConstraints() {
            ...
            for (int i = 0; i < _count; ++i) {
                ...
                TSVector2 localCenterA = pc.localCenterA;
                FixedPoint mA = pc.invMassA;
                FixedPoint iA = pc.invIA;
                TSVector2 localCenterB = pc.localCenterB;
                FixedPoint mB = pc.invMassB;
                FixedPoint iB = pc.invIB;
                ...
                for (int j = 0; j < pointCount; ++j) {
                    ...
                    minSeparation = TSMath.Min(minSeparation, separation);

                    // Prevent large corrections and allow slop.
                    FixedPoint C = MathUtils.Clamp(Settings.Baumgarte * (separation + Settings.LinearSlop),
                        -Settings.MaxLinearCorrection, 0.0f);

                    // Compute the effective mass.
                    FixedPoint rnA = MathUtils.Cross(rA, normal);
                    FixedPoint rnB = MathUtils.Cross(rB, normal);
                    FixedPoint K = mA + mB + iA * rnA * rnA + iB * rnB * rnB;

                    // Compute normal impulse
                    FixedPoint impulse = K > 0.0f ? -C / K : 0.0f;
        ...
        private static class PositionSolverManifold
        {
            public static void Initialize(ContactPositionConstraint pc, Transform xfA, Transform xfB, int index,
                out TSVector2 normal, out TSVector2 point, out FixedPoint separation) {
