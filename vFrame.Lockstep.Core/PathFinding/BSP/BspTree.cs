...
//            var perp = new TSVector2(-dir.y, dir.x);
...
//            return CreateGo(name, vertices, tirs, color);
        ...
        GameObject CreateGo(string name, List<TriRef> allRawTriangle, Color color) {
            ...
            foreach (var tri in allRawTriangle) {
                vertices.Add(new Vector3(tri.a.x.AsFloat(), 0, tri.a.y.AsFloat()));
                vertices.Add(new Vector3(tri.b.x.AsFloat(), 0, tri.b.y.AsFloat()));
                vertices.Add(new Vector3(tri.c.x.AsFloat(), 0, tri.c.y.AsFloat()));
                tirs.Add(triIdx++);
                tirs.Add(triIdx++);
                tirs.Add(triIdx++);
            }
