...
{
    public static TSVector V3_UP = TSVector.up;
    public static TSVector V3_DOWN = TSVector.down;
    private static readonly TSVector tmp1 = new();

    private readonly Plane crossingPlane = new(); // 横跨平面
    private TSVector end; // 终点
    private TriangleEdge lastEdge; // 最后一个边
    private EdgePoint lastPointAdded; // 最后一个边点
    private List<IConnection<Triangle>> nodes; // 路径连接点
    private readonly List<EdgePoint> pathPoints = new();
    private TSVector start; // 起点
    private Triangle startTri; // 起始三角形
    private readonly List<TSVector> vectors = new(); // 路径坐标点
    ...
    public TSVector getVector(int index) {
     ...
     */
    public List<TSVector> getVectors() {
        return vectors;
    }
    ...
    private void addPoint(TSVector point, Triangle toNode) {
