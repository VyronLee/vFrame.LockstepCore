...
namespace vFrame.Lockstep.Core.RVO2
{
    ...
    internal class KdTree
    {
        ...
        {
            private FixedPoint a_;
            private FixedPoint b_;
         ...
         * tested.</param>
         */
        internal bool queryVisibility(TSVector2 q1, TSVector2 q2, FixedPoint radius)
        {
        ...
        private void buildAgentTreeRecursive(int begin, int end, int node)
        {
            ...
            if (end - begin > MAX_LEAF_SIZE)
            {
                ...
                FixedPoint splitValue = 0.5f * (isVertical ? agentTree_[node].maxX_ + agentTree_[node].minX_ : agentTree_[node].maxY_ + agentTree_[node].minY_);
        ...
        private void queryAgentTreeRecursive(Agent agent, ref FixedPoint rangeSq, int node)
        {
            ...
            {
                FixedPoint distSqLeft =
                    RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].left_].minX_ - agent.position_.x))
                    + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.x - agentTree_[agentTree_[node].left_].maxX_))
                    + RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].left_].minY_ - agent.position_.y))
                    + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.y - agentTree_[agentTree_[node].left_].maxY_));

                FixedPoint distSqRight =
                    RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].right_].minX_ - agent.position_.x))
                    + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.x - agentTree_[agentTree_[node].right_].maxX_))
                    + RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].right_].minY_ - agent.position_.y))
                    + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.y - agentTree_[agentTree_[node].right_].maxY_));
         ...
         * <param name="node">The current obstacle k-D node.</param>
         */
        private bool queryVisibilityRecursive(TSVector2 q1, TSVector2 q2, FixedPoint radius, ObstacleTreeNode node)
        {
            if (node == null)
            {
                return true;
            ...
            FixedPoint invLengthI = 1.0f / RVOMath.absSq(obstacle2.point_ - obstacle1.point_);
            ...
            FixedPoint invLengthQ = 1.0f / RVOMath.absSq(q2 - q1);
