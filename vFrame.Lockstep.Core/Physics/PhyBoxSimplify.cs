...
namespace vFrame.Lockstep.Core.Physics2D
{
    ...
    public class MergeShape
    {
        ...
        public MergeShape(string name, int layer, uint source, TSVector2 center, TSVector2 forward, Vertices vertex,
            bool loop, bool canMerge = true) {
            m_center = center;
            m_forward = forward;
    ...
    public class PhyBoxSimplify
    {
        ...
        public void AddBox(string name, TSVector2 center, TSVector2 forward, TSVector2 size, int layer,
            bool canMerge = true) {
            var extend = size * FixedPoint.Half;
            var shape = new MergeShape(name, layer, m_nextSource++, center, forward,
                PolygonTools.CreateRectangle(extend.x, extend.y), true, canMerge);
