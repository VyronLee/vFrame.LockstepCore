...
namespace vFrame.Lockstep.Core.Physics2D
{
    public static class CuttingTools
    {
        ...
        public static void SplitShape(Fixture fixture, TSVector2 entryPoint, TSVector2 exitPoint, out Vertices first,
            out Vertices second) {
            ...
            for (int i = 0; i < vertices.Count; i++) {
                ...
                if (TSVector2.Dot(MathUtils.Cross(localExitPoint - localEntryPoint, 1), vertices[i] - localEntryPoint) >
                    Settings.Epsilon)
                    n = 0;
                else
                    n = 1;
            ...
            for (int i = 0; i < fixtures.Count; i++) {
                ...
                if (fixtures[i].Body.BodyType != BodyType.Static) {
                    ...
                    if (first.CheckPolygon() == PolygonError.NoError) {
                        Body firstFixture = BodyFactory.CreatePolygon(world, first, fixtures[i].Body.Position);
                        firstFixture.Forward = fixtures[i].Body.Forward;
                        firstFixture.BodyType = BodyType.Dynamic;
                    }

                    if (second.CheckPolygon() == PolygonError.NoError) {
                        Body secondFixture = BodyFactory.CreatePolygon(world, second, fixtures[i].Body.Position);
                        secondFixture.Forward = fixtures[i].Body.Forward;
