        ...
        public void CircleCast(Func<CircleCastInput, int, int, FixedPoint> callback, ref CircleCastInput input,
            ...
            while (_raycastStack.Count > 0) {
                ...
                if (node.IsLeaf()) {
                    ...
                    subInput.SweepInput = input.SweepInput;
                    ...
                    {
                        _nodes[node.Child1].ParentDebug = nodeId;
                    }
                    if (node.Child2 != NullNode)
                    {
                        _nodes[node.Child2].ParentDebug = nodeId;
                    }
#endif
                    _raycastStack.Push(node.Child1);
                    _raycastStack.Push(node.Child2);
...
#if DOD_DEBUG
            _nodes[nodeId].ParentDebug = NullNode;
#endif
