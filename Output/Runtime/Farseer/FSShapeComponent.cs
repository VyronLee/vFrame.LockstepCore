...
public class FSShapeComponent : MonoBehaviour
{
	...
	protected PolygonShape GetPolyShape()
	{
		...
		List<TSVector2> vs = new List<TSVector2>();
		if(UseUnityCollider)
		{
			...
			//v03.Scale(scale);
			vs.Add(FSHelper.Vector3ToTSVector2(v00));
			vs.Add(FSHelper.Vector3ToTSVector2(v01));
			vs.Add(FSHelper.Vector3ToTSVector2(v02));
			vs.Add(FSHelper.Vector3ToTSVector2(v03));
		}
		else
		{
			if(PolygonPoints.Length < 3)
			...
			for(int i = 0; i < PolygonPoints.Length; i++)
			{
				if(!isBody)
				{
					...
					vs.Add(FSHelper.Vector3ToTSVector2(FSHelper.LocalTranslatedVec3(localp, this.transform.parent)));
					...
					vs.Add(FSHelper.Vector3ToTSVector2(PolygonPoints[i].localPosition));
			}
		}
