...
public class FSPulleyJointComponent : FSJointComponent
{
	...
	public override void InitJoint ()
	{
		...
		joint = JointFactory.CreatePulleyJoint(FSWorldComponent.PhysicsWorld,
			BodyA.PhysicsBody,
			BodyB.PhysicsBody,
			FSHelper.Vector3ToTSVector2(GroundAnchorA.position),
			FSHelper.Vector3ToTSVector2(GroundAnchorB.position),
			BodyA.PhysicsBody.GetLocalPoint(FSHelper.Vector3ToTSVector2(p0)),
			BodyB.PhysicsBody.GetLocalPoint(FSHelper.Vector3ToTSVector2(p1)),
			ForceRatio);
