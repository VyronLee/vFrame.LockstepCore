    ...
    public class EditorNavMeshTool : UnityEditor.Editor
    {
        ...
        void CreateMapTestMesh() {
            ...
            UnWalkMesh.vertices = new Vector3[] {
                new TSVector(_property.startX, FixedPoint.Zero, _property.startZ).ToVector3(),
                new TSVector(_property.startX, FixedPoint.Zero, _property.endZ + _property.startZ).ToVector3(),
                new TSVector(_property.endX + _property.startX, FixedPoint.Zero, _property.endZ + _property.startZ)
                    .ToVector3(),
                new TSVector(_property.endX + _property.startX, FixedPoint.Zero, _property.startZ).ToVector3()
            };
        ...
        static string MeshToString(UnityEngine.AI.NavMeshTriangulation mesh, int type) {
            ...
            for (int i = 0; i < mesh.vertices.Length; i++) {
                ...
                sb.Append("{\"x\":").Append(v.x).Append(",\"y\":").Append(type == 0 ? 0 : v.y).Append(",\"z\":")
                    .Append(v.z)
                    .Append("},");
            }
