...
#pragma warning restore 618
	...
	protected virtual void ConvertToConvex(FSConcaveShapeComponent targetCSC)
	{
		...
		// convert vertices
		Vertices concaveVertices = new Vertices();

		if(targetCSC.PointInput == FSShapePointInput.Transform)
		{
			for(int i = 0; i < targetCSC.TransformPoints.Length; i++)
			{
				concaveVertices.Add(FSHelper.Vector3ToTSVector2(targetCSC.TransformPoints[i].localPosition));
			}
		}
		List<Vertices> convexShapeVs = BayazitDecomposer.ConvexPartition(concaveVertices);
		...
		for(int i = 0; i < convexShapeVs.Count; i++)
		{
			...
			for(int j = 0; j < convexShapeVs[i].Count; j++)
			{
				...
				pnew.transform.localPosition = FSHelper.TSVector2ToVector3(convexShapeVs[i][j]);
